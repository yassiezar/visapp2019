...
The work here presented is part of a project to assist the visually impaired (VI) navigate and find objects in unknown environments with the aid of a smartphone. The proposed system implements ideas from the field of active vision~\cite{Bajcsy2017}, but replaces the typical electro-mechanical actuators of the moving camera with the body (i.e. arm, hand) of the user holding the smartphone. Schematically, the actuators used in a typical control loop, pictured in Figure~\ref{fig:control-loop}, are replaced by a human. %This project expands upon concepts originally proposed in~\cite{bellotto2013} and~\cite{lock2017portable}.
...
\begin{figure}
  ...
  \begin{subfigure}
    ...
    \caption{The ActiVis system in use during an experiment. }\label{fig:system-use}
  \end{subfigure}
  \begin{subfigure}
    \centering
  ...
  \end{subfigure}
  \includegraphics[width=0.45\textwidth]{figures/control_loop.png}
  \caption{The ActVis system layout and control loop. \todo{Explain the meanings of the symbols r, e, ... in the caption. Remove all direct references to ActiVis with more generic ``our project''.}}\label{fig:control-loop}
\end{figure}
