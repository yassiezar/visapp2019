...
The app was developed on and experimented with an Asus ZenPhone AR\todo{No footers}\footnote{asus.com/uk/Phone/ZenFone-AR-ZS571KL/} running Android version 7.0 with the ARCore\footnote{https://developers.google.com/ar/} SDK enabled. No hardware or software modifications were required.
...
Table~\ref{tab:results-full} shows a fairly consistent spread for the TAR across the participants. The inter-participant spread ($\sigma=11\%$) is fairly significant, perhaps indicating that the user's search behaviour and strategy affects the target acquisition performance, but with an average TAR of $81.9\%$, it is clear that the system successfully finds the target object during the vast majority of searches. 
