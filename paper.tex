...
\begin{itemize}
  ...
  \item an MDP model that understands the spatial relationship between objects that is based on real data and
  \item experiments that prove the effectiveness of the current system implementation.
\end{itemize}
...
\begin{figure}
  ...
  \caption{An example of a guidance strategy the MDP will generate. }\label{fig:route-example}
\end{figure}
...
We set the model to track a maximum of of 9 steps to the target, with 9 being the longest possible route on the grid (more details about the grid are in Section~\ref{sec:system-implementation}). A search can take longer than 9 steps, but the MDP considers anything greater than 9 steps as the maximum, which is convenient for keeping a manageable state-space and a simple reward function. The MDP therefore has a total of 144 reachable states ($n_{states}=8\times9\times2$).
